    ...
    .template-result
      ...
      .result-footer
        router-link.nav-left.button-primary.button-lg(v-bind:to="{ name : 'Question' }", tag='button')
          ...
          span {{ again }}
        router-link.nav-right.button-primary.button-lg(v-bind:to="{ name: 'Networks', params: { id: $route.params.id, s: $route.params.s }}", tag='button')
          span {{ network }}
  ...
  methods: {
    setMeta: function() {
      ...
      this.results.forEach(function(v, i) {
        if (i == id)
          title = 'わたしは「' + v.name_secondary + '」タイプ！｜ナビスコラ：学問分野診断＆相関図';
      });
      ...
      results: [
        {
          ...
          'id': 5,
          'path': 'mathematical-and-physical-sciences',
          ...
          'id': 6,
          'path': 'general-science-and-engineering',
